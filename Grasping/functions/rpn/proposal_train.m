...
%% -------------------- Training -------------------- 
    ...
    while (iter_ < max_iter)
        ...
        train_results2=train_results;
        % do valdiation per val_interval iterations
        if ~mod(iter_,400)
            Trex=Trex+1;
            if opts.do_val
                val_results2 = do_validation(conf, caffe_solver, proposal_generate_minibatch_fun, image_roidb_val, shuffled_inds_val);
            end
            [Phantom(Trex,:),Phantom2(Trex,:)]=show_state2(train_results2, val_results2);
            train_results2=[];
            val_results2=[];
        end
...
function [Training,Testing]=show_state2(train_results, val_results)
       Training(1,:)=[1 - mean(train_results.accuracy_fg.data), 1 - mean(train_results.accuracy_bg.data), ...
        mean(train_results.loss_cls.data), ...
        mean(train_results.loss_bbox.data),1];
    if exist('val_results', 'var') && ~isempty(val_results)
        Testing(1,:)=[1 - mean(val_results.accuracy_fg.data), 1 - mean(val_results.accuracy_bg.data), ...
            mean(val_results.loss_cls.data), ...
            mean(val_results.loss_bbox.data),1];
    else 
        Testing(1,:)=[0,0,0,0,0];
    end
end
