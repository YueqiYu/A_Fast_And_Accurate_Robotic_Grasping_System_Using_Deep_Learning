            ...
            Trex=Trex+1;
             if opts.do_val
                ...
                for i = 1:length(shuffled_inds_val)
                    ...
                    rst = caffe_solver.net.get_output();
                     val_results2 = parse_rst(val_results, rst);
                end
            end
...
function [Phantom,Phantom2]=show_state2(train_results, val_results)
  Phantom(1,:)=[ 1 - mean(train_results.accuarcy.data), ...
        mean(train_results.loss_cls.data), ...
        mean(train_results.loss_bbox.data),1];
    if exist('val_results', 'var') && ~isempty(val_results)
        Phantom2(1,:)=[1 - mean(val_results.accuarcy.data), ...
            mean(val_results.loss_cls.data), ...
